...
export default {
    components: { FileBrowser, SearchResult, MduiDialog, MduiCheckbox, MduiBtn, CreateDownloadDialog },
    name: 'FileHandler',
    props: {
        uid: {
            type: Number
    ...
    methods: {
        async createDownload(task) {
            ...
            if (!task.url) {
                mdui.alert('下载URL不能为空', () => { this.showDownload = true })
                return
            }
            ...
            try {
                ...
                this.loading = false
                mdui.alert('任务创建成功，保存位置：' + conf.data.savePath)
            } catch (error) {
